...
| `annotations` | [`ToolAnnotations`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#tool-annotations)`| None` | Optional MCP tool annotations providing behavioral hints to clients |
...
| `tag` | `str | None` | User-set session tag (see [`tag_session()`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#tag-session)) |
...
Returns [`SDKSessionInfo`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#return-type-sdk-session-info), or `None` if the session is not found.
...
Rename the most recent session so it’s easier to find later. The new title appears in [`SDKSessionInfo.custom_title`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#return-type-sdk-session-info) on subsequent reads.
...
| `get_mcp_status()` | Get the status of all configured MCP servers. Returns [`McpStatusResponse`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#mcp-status-response) |
...
| `stop_task(task_id)` | Stop a running background task. A [`TaskNotificationMessage`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#task-notification-message) with status `"stopped"` follows in the message stream |
...
| `betas` | `list[SdkBeta]` | `[]` | Beta features to enable. See [`SdkBeta`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#sdk-beta) for available options |
...
| `env` | `dict[str, str]` | `{}` | Environment variables merged on top of the inherited process environment. See [Environment variables](https://code.claude.com/docs/en/env-vars) for variables the underlying CLI reads |
...
| `can_use_tool` | [`CanUseTool`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#can-use-tool)`| None` | `None` | Tool permission callback function. See [Permission types](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#can-use-tool) for details |
| `hooks` | `dict[HookEvent, list[HookMatcher]] | None` | `None` | Hook configurations for intercepting events |
| `user` | `str | None` | `None` | User identifier |
| `include_partial_messages` | `bool` | `False` | Include partial message streaming events. When enabled, [`StreamEvent`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#stream-event) messages are yielded |
...
| `sandbox` | [`SandboxSettings`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#sandbox-settings)`| None` | `None` | Configure sandbox behavior programmatically. See [Sandbox settings](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#sandbox-settings) for details |
...
| `thinking` | [`ThinkingConfig`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#thinking-config)`| None` | `None` | Controls extended thinking behavior. Takes precedence over `max_thinking_tokens` |
| `effort` | `Literal["low", "medium", "high", "max"] | None` | `None` | Effort level for thinking depth |
| `session_store` | [`SessionStore`](https://code.claude.com/docs/en/agent-sdk/session-storage#the-session-store-interface)`| None` | `None` | Mirror session transcripts to an external backend so any host can resume them. See [Persist sessions to external storage](https://code.claude.com/docs/en/agent-sdk/session-storage) |
...
| `permissionMode` | No | Permission mode for tool execution within this agent. See [`PermissionMode`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#permission-mode) |
...
`McpSdkServerConfigStatus` is the serializable form of [`McpSdkServerConfig`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#mcp-sdk-server-config) with only `type` (`"sdk"`) and `name` (`str`) fields; the in-process `instance` is omitted. `McpClaudeAIProxyServerConfig` has `type` (`"claudeai-proxy"`), `url` (`str`), and `id` (`str`) fields.
...
Status of a connected MCP server, contained in [`McpStatusResponse`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#mcp-status-response).
...
| `config` | [`McpServerStatusConfig`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#mcp-server-status-config) (optional) | Server configuration. Same shape as [`McpServerConfig`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#mcp-server-config) (stdio, SSE, HTTP, or SDK), plus a `claudeai-proxy` variant for servers connected through claude.ai |
...
| `error` | [`AssistantMessageError`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#assistant-message-error)`| None` | Error type if the response encountered an error |
| `usage` | `dict[str, Any] | None` | Per-message token usage (same keys as [`ResultMessage.usage`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#result-message)) |
...
| Field | Type | Description |
| --- | --- | --- |
| `rate_limit_info` | [`RateLimitInfo`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#rate-limit-info) | Current rate limit state |
| `uuid` | `str` | Unique event identifier |
| `session_id` | `str` | Session identifier |

### `RateLimitInfo`

Rate limit state carried by [`RateLimitEvent`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#rate-limit-event).
...
*   `input`: Strongly-typed hook input with discriminated unions based on `hook_event_name` (see [`HookInput`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#hook-input))
*   `tool_use_id`: Optional tool use identifier (for tool-related hooks)
*   `context`: Hook context with additional information

Returns a [`HookJSONOutput`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#hook-json-output) that may contain:
...
| `network` | [`SandboxNetworkConfig`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#sandbox-network-config) | `None` | Network-specific sandbox configuration |
| `ignoreViolations` | [`SandboxIgnoreViolations`](https://docs.anthropic.com/en/docs/claude-code/sdk/sdk-python#sandbox-ignore-violations) | `None` | Configure which sandbox violations to ignore |
