...
Run `/config` and select **Output style** to pick a style from a menu. Your selection is saved to `.claude/settings.local.json` at the [local project level](https://code.claude.com/docs/en/settings).To set a style without the menu, edit the `outputStyle` field directly in a settings file:
...
You can save these files at the user level (`~/.claude/output-styles`) or project level (`.claude/output-styles`). [Plugins](https://code.claude.com/docs/en/plugins-reference) can also ship output styles in an `output-styles/` directory.
