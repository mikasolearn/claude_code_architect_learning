...
| `tools` | No | [Tools](https://docs.anthropic.com/en/docs/claude-code/sub-agents#available-tools) the subagent can use. Inherits all tools if omitted |
| `disallowedTools` | No | Tools to deny, removed from inherited or specified list |
| `model` | No | [Model](https://docs.anthropic.com/en/docs/claude-code/sub-agents#choose-a-model) to use: `sonnet`, `opus`, `haiku`, a full model ID (for example, `claude-opus-4-7`), or `inherit`. Defaults to `inherit` |
...
To disable all background task functionality, set the `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS` environment variable to `1`. See [Environment variables](https://code.claude.com/docs/en/env-vars).When [fork mode](https://docs.anthropic.com/en/docs/claude-code/sub-agents#fork-the-current-conversation) is enabled, every subagent spawn runs in the background regardless of the `background` field. Forks still surface permission prompts in your terminal as they occur instead of pre-approving; named subagents follow the pre-approval flow above.
...
*   Claude spawns a fork whenever it would otherwise use the [general-purpose](https://docs.anthropic.com/en/docs/claude-code/sub-agents#built-in-subagents) subagent. Named subagents such as Explore still spawn as before.
...
| Permissions | Prompts surface in your terminal | [Pre-approved](https://docs.anthropic.com/en/docs/claude-code/sub-agents#run-subagents-in-foreground-or-background) before launch, then auto-denied |
