...
In addition to the [common input fields](https://docs.anthropic.com/en/docs/claude-code/hooks#common-input-fields), SessionStart hooks receive `source`, `model`, and optionally `agent_type`. The `source` field indicates how the session started: `"startup"` for new sessions, `"resume"` for resumed sessions, `"clear"` after `/clear`, or `"compact"` after compaction. The `model` field contains the model identifier. If you start Claude Code with `claude --agent <name>`, an `agent_type` field contains the agent name.
...
SubagentStop hooks use the same decision control format as [Stop hooks](https://docs.anthropic.com/en/docs/claude-code/hooks#stop-decision-control).
